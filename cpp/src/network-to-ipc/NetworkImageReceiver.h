...
#include <InteractiveToolkit/Platform/SocketUDP.h>

#include <InteractiveToolkit/ITKCommon/StringUtil.h>

#include <InteractiveToolkit/Platform/Core/NetworkConstants.h>
...
class NetworkImageReceiver : public EventCore::HandleCallback
{

    Platform::Thread thread;

public:
    EventCore::Callback<void(
        const uint8_t *data, size_t data_size,
        uint32_t width, uint32_t height,
        uint32_t format)>
        onData;
    ...
    NetworkImageReceiver() : thread(
                                 EventCore::CallbackWrapper(&NetworkImageReceiver::run, this))
    {
    }
    ...
    void run()
    {
        ...
        Platform::SocketUDP udp_discover_smartphone;
        Platform::SocketTCP tcp_data_receiver;
        ...
        while (!Platform::Thread::isCurrentThreadInterrupted())
        {
            ...
            if (force_connection_to_ip.size() == 0)
            {
                ...
                udp_discover_smartphone.bind("INADDR_ANY", Platform::NetworkConstants::PUBLIC_PORT_MULTICAST_START);
                ...
                if (!ITKCommon::StringUtil::startsWith(input_data, "OpenMultimediaVideoSource:"))
                {
            ...
            if (!tcp_data_receiver.connect(device_addr, Platform::NetworkConstants::PUBLIC_PORT_START))
            {
            ...
            while (!tcp_data_receiver.isClosed() && !tcp_data_receiver.isSignaled())
            {
                ...
                { // all other formats are compressed... so the size must be less than the original image size.
