...
//    private static CameraReference.Size size;
    ...
    */
    ...
    public static void OnTouch(float x, float y) {
        if (!isOpened())
            return;
        mCamera.touchOnImageToFocus(x,y);
    }

    public static Rect GetSensorArea() {
        if (!isOpened())
            return new Rect(0,0,32, 32);
        return mCamera.getSensorArea();
    }
    ...
    public static void configureCamera(int width, int height, boolean flash, int[] fpsRange, int imageFormat, boolean touchToFocus) {
        if (!isOpened() || isPreviewing() )
            return;

        mCamera.setParameters(width,height,flash,fpsRange, imageFormat, touchToFocus);
    }
