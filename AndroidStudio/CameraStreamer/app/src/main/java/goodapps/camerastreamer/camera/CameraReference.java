...
public class CameraReference {
    ...
    public static boolean IsSupportedManualFocus(Context context, int cameraID) {
        if (CameraHelper.CanUseNewCameraAPI()) {
            ...
            try {
                ...
                boolean isTouchFocusSupported = false;
    ...
    void setParameters(int width, int height, boolean flash, int[] fpsRange, int imageFormat, boolean touchFocus) {
    ...
    public Rect getSensorArea() {
        if (CameraHelper.CanUseNewCameraAPI()) {
            return characteristics.get(CameraCharacteristics.SENSOR_INFO_ACTIVE_ARRAY_SIZE);
        } else {
            return new Rect(0,0,param_width, param_height);
        }
    }
    public void touchOnImageToFocus(float pX, float pY) {
        if (!param_touchFocus)
            return;

        //Size size = getSizeForTexture();
        Log.i(TAG, "Auto Focus on: " + Float.toString(pX) + ", " + Float.toString(pY) );
        //Log.i(TAG, "Tex Size: " + Float.toString(size.width) + ", " + Float.toString(size.height) );
        if (CameraHelper.CanUseNewCameraAPI()) {
            if (new_camera == null)
                return;
            ...
            int sensorWidth = param_width;
            int sensorHeight = param_height;

            if (sensorArraySize != null) {
                sensorWidth = sensorArraySize.width();
                sensorHeight =sensorArraySize.height();
            }

            int positionX = (int)((float)sensorWidth * pX);
            int positionY = (int)((float)sensorHeight * pY);
            positionX = (positionX<0)?0:((positionX>=sensorWidth)?sensorWidth-1:positionX);
            positionY = (positionY<0)?0:((positionY>=sensorHeight)?sensorHeight-1:positionY);
            Log.i(TAG, " Image position -> " + Integer.toString(positionX) + ", " + Integer.toString(positionY) );


            final int halfTouchLength  = 150;
            MeteringRectangle focusArea = new MeteringRectangle(
                    Math.max(positionX - halfTouchLength, 0),
                    Math.max(positionY - halfTouchLength, 0),
                    halfTouchLength * 2,
                    halfTouchLength * 2,
                    MeteringRectangle.METERING_WEIGHT_MAX - 1);


            AutoFocusAdapter autoFocusAdapter = new AutoFocusAdapter(this);
            ...
            try {
                ...
                cameraCaptureSession.capture(captureRequestBuilder.build(), autoFocusAdapter, new Handler(auxiliaryThread.getLooper()));
...
*/
    ...
    @RequiresApi(api = Build.VERSION_CODES.LOLLIPOP)
    static
    class AutoFocusAdapter extends CameraCaptureSession.CaptureCallback {
        CameraReference init;
        public AutoFocusAdapter(CameraReference init){
            super();
            this.init = init;
        }

        @Override
        public void onCaptureCompleted(CameraCaptureSession cameraCaptureSession, CaptureRequest request, TotalCaptureResult result) {
            super.onCaptureCompleted(cameraCaptureSession, request, result);
            this.init.cameraCaptureSession = cameraCaptureSession;
            //mManualFocusEngaged = false;

            Log.i(TAG, "AutoFocusAdapter Entering...");

            if ( request.getTag() != null && request.getTag().equals("FOCUS_TAG") ) {
                Log.i(TAG, "AutoFocusAdapter FOCUS_TAG tag found!!!");
                //the focus trigger is complete -
                //resume repeating (preview surface will get frames), clear AF trigger
                this.init.captureRequestBuilder.set(CaptureRequest.CONTROL_AF_TRIGGER, CameraMetadata.CONTROL_AF_TRIGGER_IDLE);
                ...
                this.init.captureRequestBuilder.setTag(null);
                ...
                Log.w(TAG, "AutoFocusAdapter onCaptureCompleted");
            }
        }
        ...
        public void onCaptureFailed(CameraCaptureSession session, CaptureRequest request, CaptureFailure failure) {
            ...
            this.init.cameraCaptureSession = session;

            Log.w(TAG, "AutoFocusAdapter onCaptureFailed" + failure);
            //mManualFocusEngaged = false;
        }
    }
