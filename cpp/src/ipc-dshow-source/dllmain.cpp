...
CUnknown* WINAPI ARibeiroSourceDevice_CreateInstance(LPUNKNOWN lpunk, HRESULT* phr) {
	...
	console->printf("ARibeiroSourceDevice_CreateInstance");
...
HRESULT register_filter() {
	...
	if (sizeof(wchar_t) == 2)
	{
		auto u32 = ITKCommon::StringUtil::utf16_to_utf32((char16_t*)achFileName);
		module_name = ITKCommon::StringUtil::utf32_to_utf8(u32);
	}
	else {
		module_name = ITKCommon::StringUtil::utf32_to_utf8((char32_t*)achFileName);
	}*/

	console->printf("[register_filter] Module Name: %s\n", wStringToString(achFileName).c_str());
	...
	for (int i = 0; i < num_filters_to_register; i++) {
		...
		if (sizeof(wchar_t) == 2)
		{
			auto u32 = ITKCommon::StringUtil::utf16_to_utf32((char16_t*)g_Templates[i].m_Name);
			filter_name = ITKCommon::StringUtil::utf32_to_utf8(u32);
		}
		else {
			filter_name = ITKCommon::StringUtil::utf32_to_utf8((char32_t*)g_Templates[i].m_Name);
		}*/

		console->printf("[register_filter]     Filter Name: %s\n", wStringToString(g_Templates[i].m_Name).c_str());
		if (SUCCEEDED(hr))
