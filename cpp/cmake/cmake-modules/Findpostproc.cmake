if(POSTPROC_INCLUDE_DIR AND POSTPROC_LIBRARIES)
	unset(POSTPROC_INCLUDE_DIR)
	unset(POSTPROC_LIBRARIES)
endif()

if (POSTPROC_ROOT)
	find_path(POSTPROC_INCLUDE_DIR libpostproc/postprocess.h PATHS ${POSTPROC_ROOT})
	find_library(POSTPROC_LIBRARIES NAMES postproc PATHS ${POSTPROC_ROOT})
else()
	find_path(POSTPROC_INCLUDE_DIR libpostproc/postprocess.h)
	find_library(POSTPROC_LIBRARIES NAMES postproc)
endif()

if(POSTPROC_INCLUDE_DIR AND POSTPROC_LIBRARIES)
	set(POSTPROC_FOUND ON)
endif()

if(POSTPROC_FOUND)
	if (NOT ${CMAKE_FIND_PACKAGE_NAME}_FIND_QUIETLY)
		MESSAGE(STATUS "Found ${CMAKE_FIND_PACKAGE_NAME} include:  ${POSTPROC_INCLUDE_DIR}")
		MESSAGE(STATUS "Found ${CMAKE_FIND_PACKAGE_NAME} library: ${POSTPROC_LIBRARIES}")
	endif()
else()
	if(${CMAKE_FIND_PACKAGE_NAME}_FIND_REQUIRED)
		MESSAGE(FATAL_ERROR "Could NOT find ${CMAKE_FIND_PACKAGE_NAME} development files")
	endif()
endif()