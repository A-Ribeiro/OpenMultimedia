...
namespace openglWrapper {
    ...
    void GLTexture::uploadBuffer(const void* buffer) {
        ARIBEIRO_ABORT(!isInitialized(),"You need to set the texture size and format before upload the buffer.");
    ...
    void GLTexture::setSize(int w, int h, GLuint format) {
        ...
        //if (!power_of_two_w || !power_of_two_h)
            //ARIBEIRO_ABORT(!GLAD_GL_ARB_texture_non_power_of_two&&!GLAD_GL_OES_texture_npot,"ARB_texture_non_power_of_two not supported.");
        ...
        if (internal_format == GL_RGB){
            ...
            ARIBEIRO_ABORT(true, "Texture wrong format...");
        }
        //ARIBEIRO_ABORT((input_data_type == GL_FLOAT && ! GLAD_GL_ARB_texture_float), "GL_ARB_texture_float not found");

        //ARIBEIRO_ABORT(( (internal_format == GL_SRGB || internal_format == GL_SRGB_ALPHA) && !GLAD_GL_EXT_texture_sRGB), "Graphics driver does not support texture sRGB.");
        //ARIBEIRO_ABORT( input_format == GL_STENCIL_INDEX && !GLAD_GL_VERSION_4_4, "VERSION_4_4 not supported.");
        //ARIBEIRO_ABORT( (input_format == GL_DEPTH_COMPONENT || input_format == GL_DEPTH_STENCIL ) && !GLAD_GL_ARB_depth_texture,"ARB_depth_texture not supported.");
...
//#if !defined(GLAD_GLES2)
        //if (id != 0)
            //ARIBEIRO_ABORT(!GLAD_GL_ARB_multitexture,"ARB_multitexture not supported.");
//#endif
    ...
    TextureBuffer GLTexture::download(TextureBuffer *output) {
        ...
        //ARIBEIRO_ABORT(!GLAD_GL_ARB_framebuffer_object,"the download texture needs GL_ARB_framebuffer_object extension..." );
    ...
    void GLTexture::writeToPNG(const char* filename, bool invertY) {
        ARIBEIRO_ABORT(input_data_type!=GL_UNSIGNED_BYTE, "writeToPNG only works with GL_UNSIGNED_BYTE data type.");
    ...
    //#if !defined(GLAD_GLES2)
        //ARIBEIRO_ABORT(!GLAD_GL_SGIS_generate_mipmap,"SGIS_generate_mipmap not supported.");
    //#endif
