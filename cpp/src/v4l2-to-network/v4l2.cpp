...
void Device::setWriteFormat(int width, int height, int fps, uint32_t pixelformat, int line_size_bytes, int image_size_bytes) {
    ...
    */
    ...
    ITK_ABORT(ioctl(fd, VIDIOC_G_FMT, &format) < 0,
    ...
    */
        ...
        ITK_ABORT(ioctl(fd, VIDIOC_S_FMT, &format) < 0,
    ...
    ITK_ABORT(ioctl(fd, VIDIOC_G_PARM, &fpsparam) < 0,
    ...
    if (fpsparam.parm.capture.timeperframe.denominator != fps){
        */
        ...
        ITK_ABORT(ioctl(fd, VIDIOC_S_PARM, &fpsparam) < 0,
                    "Failed to set camera FPS: %s\n", strerror(errno))
    //}
...
bool Device::queueBuffer(int bufferindex, v4l2_buffer *bufferinfo)
{
    if (fd < 0)
    {
        fprintf(stderr, "Device filedescriptor closed (%s).\n", path.c_str());
...
bool Device::dequeueBuffer(v4l2_buffer *bufferinfo)
{
    if (fd < 0)
    {
        fprintf(stderr, "Device filedescriptor closed (%s).\n", path.c_str());
