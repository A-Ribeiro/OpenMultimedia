...
std::string wStringToString(const wchar_t* str_) {
    std::wstring wstr;
    if (str_ == NULL)
        wstr = L"";
    else 
        wstr = str_;
    return ITKCommon::StringUtil::wString_to_String(wstr);
}
...
CSource(
    wStringToString(base_template->m_Name).c_str(),//base_template->m_Name,
...
CSourceStream(NAME("Video"), phr, pParent, pPinName)
{
        ...
        if (hResLoad != NULL) {
            ...
            if (pMyBinaryData != NULL) {
                ...
                reader.readFromBuffer(
                    Platform::ObjectBuffer((uint8_t*)pMyBinaryData, myResourceSize),
                    true
                );
                reader.readBuffer(&data_buffer);
    ...
    std::string queueName = wStringToString(parent->base_template->m_Name);

    // 8 buffers of 1920x1080x2 (yuyv2)
    data_queue = new LowLatencyQueueIPC(queueName.c_str(), QueueIPC_READ, 8, 1920*1080*2, false);
...
HRESULT ARibeiroOutputStream::FillBuffer(IMediaSample *ptrMediaSample)
{
    ...
    if (process == NULL) {
        ...
        if (GetModuleFileNameA(hDllModuleDll, achTemp, sizeof(achTemp)))
        {
            ...
            std::string dll_path = ITKCommon::Path::getExecutablePath(achTemp);
            parent->debug->printf("  [ARibeiroOutputStream] dll path: %s\n", dll_path.c_str());
            process = new Platform::Process(
                dll_path + "/network-to-ipc/network-to-ipc",
                ITKCommon::StringUtil::parseArgv("-noconsole")
            );
...
void ARibeiroOutputStream::sleepToSync(REFERENCE_TIME *current, const REFERENCE_TIME &target) {
    ...
    if (delta % 10000 > 0)
        delta_ms += 1;
    Platform::Sleep::millis(delta_ms);
